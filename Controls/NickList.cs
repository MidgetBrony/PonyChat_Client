...
namespace IceChat
{
    public partial class NickList : UserControl
    {
        ...
        {
            System.Diagnostics.Debug.WriteLine("OnScroll:" + e.NewValue + ":" + e.OldValue);
            topIndex = e.NewValue;
            Invalidate();
        }
                ...
                if (currentWindow != null && currentWindow.WindowStyle == IceTabPage.WindowType.Channel)
                {
                    ...
                    if (FormMain.Instance.IceChatColors.RandomizeNickColors == true)
                    {
                        for (int i = 0; i < sortedNickNames.Count; i++)
                        {
                            ...
                            currentWindow.GetNick(sortedNickNames[i].nick).nickColor = randColor;
                        }
                    }
                    ...
                    for (int i = topIndex; i < sortedNickNames.Count; i++)
                    {
                        ...
                        foreach (IPluginIceChat ipc in FormMain.Instance.LoadedPlugins)
                        {
                            if (ipc.Enabled == true)
                                args = ipc.NickListDraw(args);
                        }
                        ...
                        //System.Diagnostics.Debug.WriteLine("check away:" + args.Nick + ":" + u.Away + ":" + sortedNickNames[i].Away);
                        /*
                        if (sortedNickNames[i].Away == true)
                        {
                            b = new SolidBrush(IrcColor.colors[FormMain.Instance.IceChatColors.ChannelAwayColor]);
                            u.nickColor = FormMain.Instance.IceChatColors.ChannelAwayColor;
                            u.Away = true;
                        }
                        else
                            u.Away = false;
                        */
                        ...
                        //draw the nickname
                        g.DrawString(args.Nick, this.Font, b, 2, currentY);
                        ...
                        if (FormMain.Instance.IceChatOptions.ShowNickHost)
                        {
                            if (currentWindow.Connection.ServerSetting.IAL.ContainsKey(u.NickName))
                            {
                                ...
                                if (args.Host.Length > 0)
                                    g.DrawString(args.Host, this.Font, b, (this.Font.SizeInPoints * 14), currentY);
                            }
                        }
