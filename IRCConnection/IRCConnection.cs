...
namespace IceChat
{
    public partial class IRCConnection
    {
        ...
        public IRCConnection(ServerSetting ss)
        {
            ...
            pongTimer = new System.Timers.Timer(60000 * ss.PongTimerMinutes);    //5 minutes
        ...
        public void BuddyListCheck()
        {
            if (serverSetting.BuddyListEnable)
            {
                ...
                {
                    //System.Diagnostics.Debug.WriteLine(ison.Length + ":" + ison);
                    ison = "ISON" + ison;
                    SendData(ison);
                }
            ...
            try
            {
                ...
                else
                {
                    if (bytesRead > 0)
                    {
                        ...
                        //ns.BeginRead(readBuffer, 0, readBuffer.Length, new AsyncCallback(OnReceivedData), ns);
        ...
        public void ConnectSocket()
        {
            ...
            try
            {
                ...
                else if (serverSetting.UseIPv6 == true)
                {
                    ...
                    //IPHostEntry hosts = Dns.Resolve(serverSetting.ServerName);
                    IPHostEntry hosts = Dns.GetHostEntry(serverSetting.ServerName);
                    //foreach (IPAddress ip in hosts.AddressList)
                    //{
                    //    System.Diagnostics.Debug.WriteLine(ip.AddressFamily.ToString() + ":" + ip.Address);
                    //}
